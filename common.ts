...
import { readableStreamToString, VimeoError, parseIfJson } from "./error";
        ...
        if (e instanceof AxiosError && e.isAxiosError) {
            try {
                const responseBody =
                    ...
                    : e.response?.data
                throw new VimeoError(e, parseIfJson(responseBody))
            } catch (innerError) {
                if (innerError instanceof ReferenceError) {
                    ...
                    throw new VimeoError(e, e.response?.data)
                }
                if (innerError instanceof VimeoError) {
                    // Got "VimeoError" from the above try block
                    throw innerError;
                }
