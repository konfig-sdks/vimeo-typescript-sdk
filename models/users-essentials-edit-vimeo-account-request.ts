/*
Vimeo API

Build something great. Vimeo's API supports flexible, high-quality video integration with your custom apps.

The version of the OpenAPI document: 3.4


NOTE: This file is auto generated by Konfig (https://konfigthis.com).
*/
import type * as buffer from "buffer"

import { UsersEssentialsEditVimeoAccountRequestVideos } from './users-essentials-edit-vimeo-account-request-videos';

/**
 * 
 * @export
 * @interface UsersEssentialsEditVimeoAccountRequest
 */
export interface UsersEssentialsEditVimeoAccountRequest {
    /**
     * The user\'s bio.
     * @type {string}
     * @memberof UsersEssentialsEditVimeoAccountRequest
     */
    'bio'?: string;
    /**
     * A list of values describing the content in the user\'s videos. Find the full list in the [/contentratings](https://developer.vimeo.com/api/reference/videos#get_content_ratings) endpoint. You must provide a comma-separated list if you\'re using a query string or an array if you\'re using JSON.
     * @type {Array<string>}
     * @memberof UsersEssentialsEditVimeoAccountRequest
     */
    'content_filter'?: Array<string>;
    /**
     * The authenticated user\'s gender.  Option descriptions:  * `f` - The user\'s preferred pronouns are she and her.  * `m` - The user\'s preferred pronouns are he and him.  * `n` - The user would rather not give preferred pronouns.  * `o` - The user\'s preferred pronouns are they and them. 
     * @type {string}
     * @memberof UsersEssentialsEditVimeoAccountRequest
     */
    'gender'?: UsersEssentialsEditVimeoAccountRequestGenderEnum;
    /**
     * The user\'s custom Vimeo URL.
     * @type {string}
     * @memberof UsersEssentialsEditVimeoAccountRequest
     */
    'link'?: string;
    /**
     * The user\'s location.
     * @type {string}
     * @memberof UsersEssentialsEditVimeoAccountRequest
     */
    'location'?: string;
    /**
     * The user\'s display name.
     * @type {string}
     * @memberof UsersEssentialsEditVimeoAccountRequest
     */
    'name'?: string;
    /**
     * The default password for all future videos that this user uploads. To use this field, the **videos.privacy.view** field must be `password`.
     * @type {string}
     * @memberof UsersEssentialsEditVimeoAccountRequest
     */
    'password'?: string;
    /**
     * 
     * @type {UsersEssentialsEditVimeoAccountRequestVideos}
     * @memberof UsersEssentialsEditVimeoAccountRequest
     */
    'videos'?: UsersEssentialsEditVimeoAccountRequestVideos;
}

type UsersEssentialsEditVimeoAccountRequestGenderEnum = 'f' | 'm' | 'n' | 'o'


